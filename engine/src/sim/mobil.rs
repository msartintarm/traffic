...
pub fn should_change(
    ...
    let incentive = (a_self_target - a_self_current) - p.politeness * (a_follower_current - a_follower_target);
...
mod tests {
    ...
    fn refuses_an_unsafe_change() {
        ...
        assert!(!should_change(&p(), 0.0, 3.0, 0.0, -6.0, false));
    }

    #[test]
    fn takes_a_clearly_beneficial_change() {
        assert!(should_change(&p(), 0.0, 2.0, 0.0, -0.5, false));
    }

    #[test]
    fn declines_a_marginal_discretionary_change() {
        assert!(!should_change(&p(), 0.0, 0.1, 0.0, 0.0, false));
    }

    #[test]
    fn accepts_a_marginal_mandatory_change_if_safe() {
        // No self-gain, slight cost, but required for routing → proceed (safe).
        assert!(should_change(&p(), 0.0, -0.2, 0.0, -1.0, true));
        // still refused if unsafe
        assert!(!should_change(&p(), 0.0, -0.2, 0.0, -5.0, true));
    }
    ...
    fn politeness_can_block_a_selfish_change() {
        ...
        assert!(!should_change(&polite, 0.0, 0.3, 0.0, -1.0, false));
    }
}
