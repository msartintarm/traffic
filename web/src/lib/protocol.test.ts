...
test("CONTROL_TYPES is the complete control set", () => {
  assert.equal(CONTROL_TYPES.size, 37);
  for (const t of ["speed", "fit", "resize", "select", "demandSources", "hover", "play", "pause", "frameBudget", "parallelRouting", "cacheSort", "stopCostRouting", "laneEvalStagger", "arterialRouting", "targetedRouting", "localitySort", "sharding", "asyncRouting", "followerLod", "shardStats", "warmup", "showCrashes", "clearCrashes", "ascii", "transit"]) {
