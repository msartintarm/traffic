...
test("CONTROL_TYPES is the complete control set", () => {
  assert.equal(CONTROL_TYPES.size, 31);
  for (const t of ["speed", "fit", "resize", "select", "demandSources", "hover", "play", "pause", "frameBudget", "parallelRouting", "cacheSort", "stopCostRouting", "laneEvalStagger", "arterialRouting", "targetedRouting", "showCrashes", "clearCrashes", "ascii", "transit"]) {
